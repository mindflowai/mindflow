...
class PineconeDatabase(Database):
    def __init__(self):
        pinecone.init(api_key="dc297606-b324-4f89-9b98-d9156b2b98a5",
              environment="us-east-1-aws")
        self.indexes = {"document": pinecone.Index("document"), "document_chunk": pinecone.Index("document")}
    ...
    def _convert_pinecone_return(self, object: dict) -> dict:
        return {**{"id": object["id"], "embedding": object["values"]}, **object["metadata"]}

    def _convert_object_to_pinecone(self, object: dict) -> Tuple:
        return (
            object["id"], object["embedding"],
    ...
    def load_bulk(self, collection: str, object_ids: List[str]) -> List[Optional[dict]]:
        ...
        for object_index in range(len(vector_list)):
            ...
            if obj_id in vectors:
                vector_list[object_index] = self._convert_pinecone_return(vectors[obj_id])
    ...
    def query(self, collection: str, vector: np.ndarray, ids: List[str], top_k = 100, include_metadata=True) -> List[dict]:
        index = self._get_index(collection)
        results = index.query(vector=vector.tolist(), filter={"id": {"$in": ids}}, top_k=top_k, include_metadata=include_metadata)
