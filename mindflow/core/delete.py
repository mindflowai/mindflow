...
def run_delete(document_paths: List[str]):
    ...
    for document in documents:
        if document is not None:
            document_chunk_ids.extend([f"{document.id}_{i}" for i in range(0, int(document.num_chunks))])
    
    if len(document_chunk_ids) == 0:
        return "No documents to delete"

    document_chunks: List[Optional[DocumentChunk]] = DocumentChunk.load_bulk(document_chunk_ids) 
    output = {}
